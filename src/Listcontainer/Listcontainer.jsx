...
export default function Listcontainer() {
    ...
    const maxId = list.reduce((max, task) => Math.max(max, task.id),0)

  return (
    <section>
        <HeaderListcontainer lastId={maxId} addItem={addTask}/>
        <div className='tilecontainer'>
            {list.map((task) => { return <ListTile item={task} deleteAction={deleteTask}/> })}   
        </div>
    </section>
  )
}
